...
Current version: **6.11.6** (tracked in `VERSION` file). v6.11.0 was the stabilization checkpoint at the close of the five-pass security-audit thread (2026-05-22 to 2026-05-24). v6.11.1 (2026-05-24) shipped the PHPUnit protective quartet (197 → 217 tests) before federation stage 1 began. **Federation stage 1 closed 2026-05-24**: v6.11.2 (schema), v6.11.3 (identity keys), v6.11.4 (`GET /api/pluriverse/identity`), v6.11.5 (`GET /api/pluriverse/openapi.json` via `zircote/swagger-php`), v6.11.6 (`inc/federation/http_sig.php`, RFC 9421 sign + verify helper — no enforcement yet). Tests at **251/251**. Helper-only surface; no endpoint enforces signatures yet (that switches on in stage 3+). See `Architecture/P2P federation/P2P federation plan v10.md` in the project vault.
